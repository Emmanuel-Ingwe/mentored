...
import type { ActionMeta, PropsValue } from 'react-select';
...
type Props = {
  ...
  isClearable?: boolean;
  onChange?: (
    option: readonly Option[] | Option | null,
    actionMeta: ActionMeta<Option>,
  ) => void;
  placeholder?: string;
};

const StyledSelect = React.forwardRef<SelectProps, Props>(
  (
    {
      options,
      defaultValue,
    ...
    return (
      <Select
        ...
        className="min-h-[2.75rem] text-gray-700"
        classNames={{
          control: (state) => {
            return cn(
              'border border-gray-300 rounded-md bg-white px-3.5 py-2.5 text-sm ring-offset-white text-sm',
              state.isFocused &&
